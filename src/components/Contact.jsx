...
export default function Contact({ userRef, listing }) {
  ...
  useEffect(() => {
    async function getCarDealer() {
      const docRef = doc(db, "users", userRef);
      const docSnap = await getDoc(docRef);
      if (docSnap.exists()) {
        setCarDealer(docSnap.data());
      } else {
        toast.error("Could not find car dealer's data");
      }
    }
    getCarDealer();
  }, [userRef]);
    ...
    <>
      {carDealer !== null && (
        <div className="flex flex-col w-full">
          <p>
            Contact {carDealer.name} for the {listing.name.toLowerCase()}
          </p>
          ...
          <a
            href={`mailto:${carDealer.email}?Subject=${listing.name}&body=${message}`}
          >
            <button
